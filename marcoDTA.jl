...
function generate_s(pp::DTAPOMDP, s::DTAstate, a::DTAaction, rng::AbstractRNG)
    ...
    add_route!(NET, R1)
    add_route!(NET, R2)
    assigns = min(max(a.assign, 0.0), 1.0)
    ...
    newnet, cost = state_step!(NET, dd, queuedict, urate, rng)
    ...
    sp = DTAstate(highway_queue, urban_queue)
    #r = reward(p, s, a, sp, u, new_demands)
    #r = reward(p, s, a, sp, u, new_demands)
    #println("end s")
    return sp
...
function observation(pp::DTAProblem, s::DTAstate, a::DTAaction, sp::DTAstate)
    ...
    urate = Dict()
    urate[p.lambda*(1.0-p.connected)] = (1, 2, p.shortest)
    NET =  create_state!(NET, dd, queuedict, urate, rng)
    time_urban = compute_time(NET, dd, NET.routes[1].id)
    time_highway = compute_time(NET, dd, NET.routes[2].id)
    dtime_urban = compute_default_time(NET, dd, NET.routes[1].id)
    dtime_highway = compute_default_time(NET, dd, NET.routes[2].id)
    hn = Normal(time_highway, 1.0)
    un = Normal(time_urban, 1.0)
    #=
    if a.assign >= 0.4
...
isterminal(p::DTAProblem, s::DTAstate) =  false #s.urban >= 800
...
function baseline_action(pp::DTAProblem, s::DTAstate)
    ...
    urate[p.lambda*(1.0-p.connected)] = (1, 2, p.shortest)
    NET =  create_state!(NET, dd, queuedict, urate, rng)
    energy_urban = compute_energy_cost(NET, NET.routes[1].id)
    energy_highway = compute_energy_cost(NET, NET.routes[2].id)
